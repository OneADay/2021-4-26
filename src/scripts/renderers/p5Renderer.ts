...
export default class P5Renderer implements BaseRenderer{
    ...
    protected draw(s) {
        if (this.animating) { 
            ...
            s.colorMode(s.HSB);
            //let x = this.delta;//(s.width / 2) + s.sin(this.delta) * 50;
            //let y = -this.delta;//(s.height / 2) + s.cos(this.delta) * 50;

            for (let i = 0; i < 10; i++) {
                let x = this.x;
                let y = (s.width / 2) + s.cos(this.delta) * (i + 1) * 2;
    
                s.noStroke();
                s.circle(x, y, 10);
                s.fill((this.delta * 2) % 255, 255, 255, 255);
            }
